...
class Graph:
    ...
    def Drive(data_metrics):
        ...
        data_dict = { 'Heuristic': heuristics, 'Word length': words_len, 'Time': times, 'Memory': memories }
        ...
        # Memory
        ax[0].grid()
        ax[0].set_xticks(np.arange(min(df['Word length']), max(df['Word length'])+1, 1.0))
        ax[0].set_xlabel('word length')  # Add an x-label to the axes.
        ax[0].set_ylabel('memory [KiB]')  # Add a y-label to the axes.
        ax[0].set_title("Memory")  # Add a title to the axes.

        # Time
        ax[1].grid()
        ax[1].set_xticks(np.arange(min(df['Word length']), max(df['Word length'])+1, 1.0))
        ax[1].set_xlabel('word length')  # Add an x-label to the axes.
        ...
        for h in heuristics:
            ...
            ax[0].bar(df_h['Word length'] + unit * width + move_unit , df_h['Memory'], label=h, width = width)  # Plot some data on the axes.
            ax[1].bar(df_h['Word length'] + unit * width + move_unit , df_h['Time'], label=h, width = width)  # Plot some data on the axes.
            unit += 1
        ...
        # Export table
        df = df.sort_values(by=['Word length','Time','Memory'], ascending=[False,True,True])
